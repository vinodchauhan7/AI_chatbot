...
export const onGetSubscriptionPlan = async () => {
    try {
        ...
        console.log('OnGetSubscriptionPlan error', err);
    }
}

export const onGetAllAccountDomain = async () => {
    try {
        const user = await currentUser();
        if (!user) return
        const data = await client.user.findUnique({
        ...
        console.log('onGetAllAccountDomain error', err);
    }
}
...
export const onIntegrateDomain = async (domain: string, icon: string) => {
  ...
  export const onUpdatePassword = async (password: string) => {
    try {
      const user = await currentUser();
      if (!user) return;
      const updatedUser = await clerkClient.users.updateUser(user?.id, {password});
      if (updatedUser) {
        return {status: 200, message: 'Password updated'}
      } 
      return {statue: 400, message: 'Some Error occured'}
    } catch (err) {
      console.log('OnupdatePassword Server action error', err);
      return {statue: 400, message: err}
    }
  }
  ...
  export const onUpdateBotName = async (
    botName: string,
    domainId: string
  ) => {
    try {
      ...
      if (update) {
        return { status: 200, message: 'Bot Name message updated' }
      }
    } catch (error) {
      console.log(error)
    }
